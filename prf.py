...
crash = df[["id", "data_inversa", "horario", "br", "km", "causa_acidente", "tipo_acidente", "classificacao_acidente", "fase_dia", "sentido_via", "condicao_metereologica", "tipo_pista", "tracado_via", "uso_solo", "ilesos", "feridos_leves", "feridos_graves", "mortos", "latitude", "longitude", "delegacia", "uop"]]
...
for index, row in crash.iterrows():
    ...
    date = str(row["data_inversa"]).split("-")
    crash.at[index, "data_inversa"] = date[2]+"/"+date[1]+"/"+date[0]
...
crash.to_csv("finalCsv/crash.csv", index=False, header=True)
